...
|------|------|------|
| 文档框架 | Fumadocs | **V1 选定** |
| Token 管线（Phase 3） | Tokens Studio + Style Dictionary / Terrazzo | Figma → JSON → 多平台；当前通过 `tokens:import` 手工导入 |
...
│  自定义 MDX：FigmaEmbed · TokenTable · IconGallery · DocsImage · PlatformTabs │
└──────────────────────────┬──────────────────────────────┘
                           ↓
┌─────────────────────────────────────────────────────────┐
│  设计源 & 工程桥接                                        │
...
- **MDX 组件扩展**：通过 `getMDXComponents()` 注册 `FigmaEmbed`、`TokenTable`、`IconGallery`、`DocsImage`（覆盖 `img`）等，契合 DS 文档模式。
...
Figma Variables 导出
  ...
  → tokens/*.{light,dark}.json（W3C DTCG）
  → 文档站 TokenTable 组件读取 JSON 渲染

Phase 3：Tokens Studio → Git PR / CI → Style Dictionary v4 或 Terrazzo → 多平台产物
```
...
| 主题 | Fumadocs 默认 `neutral.css`；`global.css` 定制紧凑排版与 sidebar 布局；明暗主题由 Fumadocs `RootProvider` 提供 |
...
- Token 构建脚本（Style Dictionary / Terrazzo）— 可扩展现有 `scripts/`（已有 `tokens:import` 手工导入），勿与当前 `icons:sync` 混淆
...
| 组件 | 职责 | 优先级 |
|------|------|--------|
| `DocsImage` | 覆盖 MDX `img`：`rounded-none` + Fancybox 同页画廊触发器 | P1 ✅ |
| `DocFancybox` | docs 页外层 `Fancybox.bind`（`@fancyapps/ui`；非 MDX block） | P1 ✅ |
| `InlineTOC` | 文内目录（Fumadocs InlineTOC 包装） | P1 ✅ |
...
> 文档页外层另用 `DocFancybox`（`src/app/docs/[[...slug]]/page.tsx`）绑定 Fancybox；规范配图放 `public/media/`，勿用 gitignore 的 `public/uploads/`。
...
| Token 与 Figma Variables 漂移 | 当前从 Figma Variables 导出后运行 `tokens:import` 并提交 JSON；Phase 3 再评估 Tokens Studio + Git PR 自动同步 |
| CMS 与 MDX schema 不一致 | frontmatter Zod 校验 + CI 构建失败即阻断 |
| Tina Cloud 依赖 | 保留 Keystatic 迁移路径；内容始终在 Git |
| 中文搜索质量 | 已接入 `Intl.Segmenter('zh-CN')`；后续按真实查询评估同义词与排序质量 |
