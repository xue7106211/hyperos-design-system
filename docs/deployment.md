...
本地常用命令（与 `package.json` 一致）：`npm install` · `npm run dev` · `npm run build`（含 `tinacms build`）· `npm run tina:build` · `npm run start` · `npm run types:check`。生产镜像构建不跑 `tinacms build`。
