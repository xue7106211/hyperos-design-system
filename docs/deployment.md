...
本地常用命令（与 `package.json` 一致）：`npm install` · `npm run dev` · `npm run build`（含 `tinacms build`）· `npm run tina:build` · `npm run start` · `npm run types:check` · `npm run icons:sync`。生产镜像构建不跑 `tinacms build`；图标资产以仓库内已提交的 `icons/manifest.json` 与 `public/icons/` 为准。
