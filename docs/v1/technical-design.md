...
| 部署 | Matrix CI + Docker 多阶段构建（`Dockerfile`）；`staging` 分支部署；生产只跑 `npx next build`（不跑 `tinacms build`） |
...
│   │   └── HyperOSLogo.tsx
│   └── lib/                # source、layout、shared、docs-version-tabs、tina-docs、cn
...
Design Token（业务 token）与 Fumadocs UI token（文档站 chrome）**分离**：业务 token 来自 `tokens/tokens.json`，不混用 `--color-fd-*`。
