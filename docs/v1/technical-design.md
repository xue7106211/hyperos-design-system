...
> **版本**：V1.0  
> **日期**：2026-07-08  
> **状态**：Phase 0–1 已实施（文档站可构建运行）；Phase 2–3 规划中
...
- 支持设计/文档同学 **低门槛维护内容**（Phase 2 计划接入 CMS；当前 V1 以 MDX + Git 为主）
...
| Token 目录页（从 `tokens/tokens.json` 渲染） | Tokens Studio → Git 自动同步（Phase 3） |
| Android / iOS **静态**代码片段展示 | 完整 npm 组件包发布流水线 |
| MDX + `meta.json` 内容维护 | TinaCMS / Keystatic 可视化编辑（Phase 2） |
...
3. **CMS 优先（Phase 2）**：内容上传与日常维护必须对非工程同学友好；V1 以 MDX + Git 为主。
...
│  自定义 MDX：FigmaEmbed · TokenTable · PlatformTabs      │
└──────────────────────────┬──────────────────────────────┘
                           ↓
┌─────────────────────────────────────────────────────────┐
│  设计源 & 工程桥接                                        │
...
- **MDX 组件扩展**：通过 `getMDXComponents()` 注册 `FigmaEmbed`、`TokenTable` 等，契合 DS 文档模式。
...
| 方案 | 角色 | 阶段 |
|------|------|------|
| **TinaCMS** | 可视化编辑 MDX，支持自定义 block（FigmaEmbed 等） | **Phase 2 首选** — 当前 V1 使用 MDX 手写 + Git |
...
| **Token 同步** | Tokens Studio → Git → 构建 | 设计 + 工程 | ⚪ Phase 3（当前为手工维护 `tokens/tokens.json`） |
...
| 部署 | GitHub Actions → 静态/Node 托管（Vercel / 内网静态服务器） |
| 主题 | `global.css` 覆盖 `--color-fd-*`；Dark mode  via `next-themes` |
...
│   ├── components/mdx/     # FigmaEmbed、TokenTable、PlatformTabs 等
│   └── lib/                # source.ts、layout.shared.tsx、shared.ts
...
- `tina/` — TinaCMS 配置（Phase 2）
- `scripts/` — Token 构建脚本（Phase 3）

> **说明**：客户端组件 **源码** 建议在独立仓库维护；本仓聚焦 **文档站 + Token + Figma 连接**。Code Connect 映射文件可放本仓或组件仓，V1 试点时按团队习惯定。
...
| `fumadocs-ui` | DocsPage、Sidebar、TOC | HyperOS 主题 CSS 变量、Layout 宽度 |
...
| 类型 | 路径示例 | 核心内容 |
|------|----------|----------|
| Foundation | `/docs/foundations/colors` | 原则说明 + TokenTable |
| Component | `/docs/components/actions/button` | FigmaEmbed + Token + PlatformTabs + Do/Don't |
| Pattern | `/docs/patterns/settings-page` | 页面级 Figma embed + 用法说明 |
| Resource | `/docs/resources/figma-library` | Figma 库链接、变更日志 |
...
2. 简介（MDX 正文；Phase 2 可接 CMS 富文本）
...
| 组件 | 职责 | 优先级 |
|------|------|--------|
| `FigmaEmbed` | 设计稿 iframe（fileKey + nodeId + theme） | P0 |
| `FigmaPrototypeEmbed` | 原型 iframe，替代 Web 交互 demo | P0 |
| `PlatformTabs` / `PlatformTab` | Android / iOS 代码 Tab 切换 | P0 |
| `TokenTable` | 从 `tokens/tokens.json` 按 group 渲染色板/间距/字体 | P0 ✅ |
...
通过 `src/app/global.css` 覆盖 Fumadocs UI token：

```css
@theme {
  --color-fd-primary: /* HyperOS 品牌色 */;
...
.dark {
  --color-fd-primary: /* HyperOS 暗色品牌色 */;
  --color-fd-background: hsl(0, 0%, 7%);
  --color-fd-foreground: hsl(0, 0%, 92%);
}
```

Design Token（业务 token）与 Fumadocs UI token（文档站 chrome）**分离**：业务 token 来自 `tokens/*.json`，不混用 `--color-fd-*`。
...
| ADR-003 | CMS 首选 TinaCMS（Phase 2） | 设计师可视化编辑 + 自定义 MDX block；V1 未接入 |
